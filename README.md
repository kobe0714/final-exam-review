...
8. **信任生成的 DOCX 一定可读** → OOXML 手写拼接容易出现 double-escaping（XML 标签被 `html.escape` 转为 `&lt;` `&gt;` 显示为乱码）。生成后必须验证：解压 ZIP，检查 `word/document.xml` 中 `&lt;w:` 计数为 0、`<w:rPr>` 标签存在、ZIP 完整性通过
9. **出问题只做局部修复不做全局排查** → DOCX 是 ZIP 包，所有条目互联。只检查 `document.xml` 不够——`_rels/.rels` 指向错误同样导致 Word 打不开。出问题后必须逐个 ZIP 条目对比 OOXML 规范，全面回溯而非局部修
10. **用运行时启发式代替类型约束** → `.startswith('<w:r')` 这种启发检查本质是"在运行时猜"，不是"在类型层面保证"。手写 XML 时应引入 sentinel class（如 `XmlString(str)`）让 `isinstance()` 能区分"纯文本"和"已构建 XML"。验证规则是安全带，不是刹车——真正的正确性来自类型系统和 API 设计
11. **复习内容求全不求精** → 做知识点清单而非真正的提纲。应该知识框架先行、题答合一、剪冗余保重点，把 50K 字符压缩到 ~15K，同一概念保留最有辨析价值的 1 道题
12. **把提纲写成题库 dump** → 把所有题目堆进去就完了。正确做法：每章统一模板（知识框架→一票否决→核心速查→典型题精析→易错提醒），计算题分层（Tier 1 完整演算，Tier 2 精简核心公式）
13. **DOCX 排版不考虑渲染效果** → blockquote `>` 在 DOCX 渲染为蓝色斜体，适合用作"一票否决"和"易错提醒"的视觉标记。表格限 2-3 列避免等宽列问题。粗体 `**` 构建段落内视觉层次
...
- "DOCX 生成了就行不用验证" → 生成后 `verify_docx()` 必须 `[PASS]`，离线环境 word 不可预览，脚本自检是唯一防线
- "这个问题修一下就好不用全局排查" → DOCX 所有 ZIP 条目互联，局部修必定遗漏
...
手写 DOCX（stdlib zipfile + xml）有两个沉默 bug——XML 合法、ZIP 合法、Word 显示为乱码或打不开：

**Bug 1：Double-Escaping 类型混淆**
- `paragraph()` 收到 `run()` 返回的 XML 字符串，但 Python `str` 类型无法区分"纯文本"和"已构建 XML"
- `html.escape()` 把 `<w:r>` 再次转义成 `&lt;w:r&gt;`，Word 将 XML 标签当文本显示
- **治标（错误）**：加 `.startswith('<w:r')` 启发检查——运行时猜，新 XML 模式来了就崩
- **治本（正确）**：引入 `XmlString(str)` sentinel class，`isinstance(x, XmlString)` 在类型层面消除歧义

**Bug 2：Package-Level 关系指向错误**
- `_rels/.rels` 错误指向了 `styles.xml` 而非 `/word/document.xml`，Word 找不到文档主体
- **治标（错误）**：手动修正 XML 字符串——裸字符串 URI 下次还会写错
- **治本（正确）**：定义 `REL_OFFICE_DOC` / `REL_STYLES` 常量，编译时绑定，拼写错误变 NameError

**核心原则：在源头解决问题，不要加验证规则打补丁**

| 做法 | 机制 | 回归风险 |
|------|------|----------|
| `.startswith('<w:r')` 启发式 | 运行时猜 | 高——任何新 XML 模式 |
| `XmlString` sentinel class | 类型系统保证 | 零——`isinstance` 是语言内建机制 |
| 裸字符串 URI | 手动复制粘贴 | 高——`styles` ≠ `officeDocument` 易混淆 |
| `REL_*` 命名常量 | 导入时绑定 | 零——拼写错误是 `NameError` |
...
2. **一票否决**（blockquote，仅对关键前提条件章）—— "如果这里搞反，整章白复习"
3. **核心速查**（领域特化表格）—— 公式条件、检验步骤、判定规则
4. **典型题精析**（代表性题目 + 答案合一）—— 同一概念只保留最有辨析价值的 1 道
5. **易错提醒**（blockquote 总结）—— 考试最容易踩的坑

**冗余剪裁策略：**
- 同一知识点多道相似的 → 保留 1 道最有辨析价值的，其余并入知识框架
- 客观题集群（同一概念反复考） → 每集群只保留精选
- 计算题分层：Tier 1 完整演算步骤（考试必考题型），Tier 2 仅核心公式和关键区别
- 目标：50K → 15K 字符，压缩 ~68%

**DOCX 渲染适配：**
- 表格限 2-3 列（避免等宽列问题）
- `>` 引用块渲染为蓝色斜体 → 用于"一票否决"和"易错提醒"的视觉标记
- `**粗体**` 构建段落内视觉层次
- `▶` 前缀替代真列表标记（真列表在 DOCX 不渲染）
